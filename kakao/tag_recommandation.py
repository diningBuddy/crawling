...
def get_tags_for_restaurant(restaurant_name, restaurant_reviews):
  ...
  prompt = f"""
    다음은 "{restaurant_name}"에 대한 리뷰 목록과 각 리뷰의 추천 수야:
    
    {reviews_text}

    각 리뷰의 추천 수(review_count)가 높을수록, 그 리뷰의 중요도를 높게 평가해. 추천 수가 많으면 그 내용이 다른 리뷰보다 더 중요하게 고려해야 해.

    위 리뷰들을 종합하여, 아래의 태그 그룹에서 각 그룹당 가장 적합한 하나의 태그를 선택해. 각 그룹에서 가장 자주 언급되거나 중요하다고 판단되는 요소를 반영하여 결정해 줘.
    ...
    리뷰 내용과 추천 수에 따라, 각 그룹에서 가장 적합한 태그 하나만 골라 반환해:
